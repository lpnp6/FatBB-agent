...
class PostgresVectorSearchStore(PostgresBM25SearchStore, VectorSearchStore):
    ...
    async def areplace_document_chunks(
        ...
        All embeddings are generated in one batched pass so that the
        number of HTTP round-trips to the embedding service is
    ...
    def _upsert_chunks(self, cursor: object, chunks: Sequence[TextChunk]) -> None:
        ...
        asyncio.run(self._aupsert_chunks(cursor, chunks))
    ...
    async def _aupsert_chunks(
        ...
        values = [
            (
                ...
                self._vector_literal(self._validate_embedding(embeddings[i])),
            )
