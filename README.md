...
This repository contains a replaceable RAG foundation. Its first production-facing implementation is BM25 text retrieval backed by PostgreSQL `pg_search`: source documents are split into chunks, persisted to PostgreSQL, and ranked by the database. Query results are returned as citeable `Evidence` objects.
